...
function generateInterpretations(dayMasterHanja, gender, birthInfo, todayDayGanHanja) {
    ...
    const financeAdvice = {
        '목': '장기적인 목표에 투자하되, 지나치게 지나치지 않도록 계획을 세워 움직이세요. 공부, 자격증, 교육 관련 수익이 커질 수 있습니다.',
        '화': '사람과의 관계 속에서 수익원이 생기기 좋은 시기입니다. 홍보, 발표, 출판 또는 강연 관련 기회가 재물을 이끌 수 있습니다.',
        '토': '부동산, 임대, 저장 가능한 자산에 유리합니다. 안정적이고 현실적인 계약을 중심으로 재정 계획을 세우세요.',
        '금': '수집, 정리, 투자 분석이 더욱 중요합니다. 금전 출납과 서류 관리를 꼼꼼히 하면 손해를 줄일 수 있습니다.',
        '수': '유동성과 현금 흐름 관리가 핵심입니다. 대출과 상환 계획을 정확히 세우고 감정 소비를 자제하세요.'
    };

    const healthAdvice = {
        '목': '허리와 간을 보호하고, 규칙적인 스트레칭과 충분한 수분을 챙기세요.',
        '화': '심장과 눈을 편안하게 하고, 과로를 피하며 휴식을 자주 취하세요.',
        '토': '소화기 계통을 주의하고, 규칙적인 식사와 적당한 걷기를 권장합니다.',
        '금': '호흡기와 목 건강을 지키고, 차가운 환경에서 몸을 따뜻하게 관리하세요.',
        '수': '신장과 비뇨기 계통을 보호하며, 수분 조절과 충분한 수면을 신경 쓰세요.'
    };

    let wealth = `2026년 당신의 재물과 건강은 ${elementNames[strongest]}의 기운을 중심으로 움직입니다. ${financeAdvice[strongest]}`;
    wealth += ` ${healthAdvice[weakest]}`;
    wealth += ` ${deficiencyAdvice}`;
    wealth += ` 현재 사주에서는 ${weakest === '수' ? '수분 섭취와 마음의 안정' : weakest === '목' ? '창의적 활동과 생활의 활력' : weakest === '화' ? '열정과 표현력의 조절' : weakest === '금' ? '규칙성과 조직력' : '중용의 자세'}에 특히 신경 쓰는 것이 좋습니다.`;
